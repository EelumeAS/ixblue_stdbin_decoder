...
{
    uint64_t bytes = *static_cast<const uint64_t*>(buf.data().data());
...
{
    uint32_t bytes = *static_cast<const uint32_t*>(buf.data().data());
    boost::endian::big_to_native_inplace(bytes);
...
{
    res = *static_cast<const int32_t*>(buf.data().data());
...
{
    res = *static_cast<const uint64_t*>(buf.data().data());
    boost::endian::big_to_native_inplace(res);
...
{
    res = *static_cast<const uint32_t*>(buf.data().data());
    boost::endian::big_to_native_inplace(res);
...
{
    res = *static_cast<const uint16_t*>(buf.data().data());
    boost::endian::big_to_native_inplace(res);
...
{
    res = *static_cast<const uint8_t*>(buf.data().data());
